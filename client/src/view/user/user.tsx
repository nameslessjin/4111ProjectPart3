...
export default class User extends React.Component<RouteChildrenProps, State> {
  ...
  componentDidMount() {
    ...
    // ok, remind me to change the query statement for get sections.  Eliminate the course user already taken if user exists
  }

  checkAuth() {
    const user_id = localStorage.getItem("user_id");
    const user_id_pathname = this.props.location.pathname.split("/user/");
    if (user_id_pathname[1] != user_id) {
  ...
  fetchCourseResult() {
    const url = http + `user/${localStorage.getItem("user_id")}`;
